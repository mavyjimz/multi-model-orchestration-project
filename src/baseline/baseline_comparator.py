...
class BaselineComparator:
    ...
    def __init__(
        self,
        calculator: RollingWindowCalculator | None = None,
    ...
    def compare(
        self, baseline_window: str = "long_term", current_window: str = "short_term"
    ) -> BaselineComparison:
        """
    ...
    def _compare_metrics(self, baseline: WindowStats, current: WindowStats) -> dict[str, Any]:
        ...
        return {
            "accuracy": {
                ...
                "delta_percentage": round(accuracy_delta / baseline_acc * 100, 2)
                if baseline_acc > 0
                else 0.0,
            },
            "latency_p99_ms": {
                ...
                "delta_percentage": round(latency_delta / baseline_lat * 100, 2)
                if baseline_lat > 0
                else 0.0,
            },
            "error_rate": {
                ...
                "delta_percentage": round(error_rate_delta / baseline_err * 100, 2)
                if baseline_err > 0
                else 0.0,
            },
    ...
    def _generate_recommendations(
        self, status: ComparisonStatus, metrics: dict[str, Any]
    ) -> list[str]:
        ...
        if status == ComparisonStatus.IMPROVED:
            ...
            recommendations.append("Document changes that led to improvement for future reference.")

        elif status == ComparisonStatus.DEGRADED:
            recommendations.append(
                "Performance degradation detected. Investigate root cause immediately."
            )
            recommendations.append("Consider triggering model retraining (Phase 11.2).")
            recommendations.append("Review recent deployments or data changes.")

        elif status == ComparisonStatus.WARNING:
            recommendations.append("Minor performance changes detected. Continue monitoring.")
            recommendations.append("Set up alerts for further degradation.")

        else:
            recommendations.append("Performance within expected baseline. No action required.")
