...
class RollingWindowCalculator:
    ...
    def _save_window_stats(self, stats: WindowStats):
        ...
        filename = self.windows_dir / f"{stats.window_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        with open(filename, 'w') as f:
            json.dump(asdict(stats), f, indent=2)
    
    def calculate_all_windows(self) -> Dict[str, Optional[WindowStats]]:
    ...
    def export_data(self, output_path: str = None) -> str:
        ...
        if output_path is None:
            output_path = str(self.data_dir / f"data_export_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json")
