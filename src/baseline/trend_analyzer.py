...
class TrendAnalyzer:
    ...
    def analyze_trend(
        ...
        alert_recommended = (trend_direction == "degrading" and trend_strength > 0.5) or (
            forecast_14d < y_mean * 0.9
            if "accuracy" in metric_name
            else forecast_14d > y_mean * 1.2
        )
    ...
    def _save_forecast(self, forecast: TrendForecast):
        ...
        filename = (
            self.forecasts_dir
            / f"forecast_{forecast.metric_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
        with open(filename, "w") as f:
            json.dump(asdict(forecast), f, indent=2)

    def analyze_all_metrics(self) -> dict[str, TrendForecast | None]:
    ...
    def generate_summary_report(self) -> dict[str, Any]:
        ...
        # Save report
        report_file = (
            self.forecasts_dir / f"trend_summary_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
