...
class TrafficRouter:
    ...
    def route_request(
        self, request_id: str | None = None, session_id: str | None = None, sticky: bool = True
    ) -> RoutingDecision:
        """
    ...
    def _sticky_route(self, session_id: str) -> dict[str, str]:
        ...
        if bucket < self.candidate_traffic_percentage:
            return {"model": self.candidate_model, "version": self.candidate_version}
        else:
            return {"model": self.baseline_model, "version": self.baseline_version}
    ...
    def get_routing_stats(self) -> dict[str, Any]:
        ...
        candidate_count = sum(
            1 for log in self.request_log if log["selected_version"] == self.candidate_version
        )
    ...
    def export_metrics(self) -> str:
        ...
        metrics_file = (
            self.metrics_dir / f"routing_stats_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
