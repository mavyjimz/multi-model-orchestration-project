...
class ModelInfo(BaseModel):
    ...
    deprecation_date: datetime | None = Field(
        None, description="Timestamp when model was marked deprecated"
    )
    deprecation_reason: str | None = Field(
        None, description="Reason for deprecation (required if deprecated)"
    )
    migration_guide: str | None = Field(None, description="Path to migration documentation")
    retirement_scheduled: bool | None = Field(False, description="Whether retirement is scheduled")
    retirement_date: datetime | None = Field(None, description="Planned retirement date")
...
class DeprecationRequest(BaseModel):
    ...
    migration_guide: str | None = Field(
        None, description="Path to migration documentation (required if policy enforces)"
    )
    effective_date: datetime | None = Field(
        None, description="When deprecation takes effect (default: now)"
    )
    notify_stakeholders: bool = Field(True, description="Whether to trigger notification workflow")
...
class RetirementRequest(BaseModel):
    ...
    soft_delete: bool = Field(True, description="Whether to soft-delete (preserve audit trail)")
    confirmation: str = Field(
        ..., pattern="^I confirm retirement$", description="Must be exactly: 'I confirm retirement'"
    )
    archive_location: str | None = Field(
        None, description="Target archive path for retired artifacts"
    )
...
class BackupRequest(BaseModel):
    ...
    components: list[str] | None = Field(
        default=None, description="List of components to backup. If None, uses policy defaults."
    )
    compression: str | None = Field(
        default="gzip", description="Compression algorithm: gzip, bzip2, or none"
    )
    ...
    dry_run: bool = Field(
        default=False, description="If true, simulate backup without writing files"
    )
    policy_override: dict[str, Any] | None = Field(
        default=None, description="Optional policy parameters to override config/backup_policy.yaml"
    )
